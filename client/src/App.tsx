...
function useTheme(): [Theme, () => void] {
  ...
  useEffect(() => {
    ...
    if (meta) meta.setAttribute('content', theme === 'dark' ? '#080a0f' : '#e9ebf1');
  }, [theme]);
...
function Shell() {
  ...
  return (
    <div className="min-h-dvh">
      <TopNav theme={theme} onToggleTheme={toggleTheme} onOpenSearch={() => setSearchOpen(true)} />
      <main className="mx-auto w-full max-w-7xl px-3 pb-24 pt-4 sm:px-5 md:pb-10" id="content">
        <Routes>
          ...
          <Route
            path="*"
            element={
              <p className="py-20 text-center text-sm text-muted">Page not found — head back to Home.</p>
            }
          />
        </Routes>
      </main>
